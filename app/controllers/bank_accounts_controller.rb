...
class BankAccountsController < AuthenticationController
  ...
  def index
    @bank_accounts = BankAccount.includes(:user, :payout_channel).all.page(params[:page]).per(params[:per_page])
  end
  ...
  def bank_account_params
    params.require(:bank_account).permit(:user_id, :payout_channel_id, :name, :number, :active)
  end
end
